...
contract CoinFlip is Ownable {
    ...
    mapping(address => PlayerByAddress) private playersByAddress; //to check who is the player
    ...
    constructor(TMNG _tmng) {
        tmng = _tmng;
    }

    function deposit(uint256 _amount) external {
        uint256 userTokenBalance = tmng.balanceOf(msg.sender);
        require(_amount < userTokenBalance, "Insufficient token balance.");
        tmng.transferFrom(msg.sender, address(this), _amount);
        contractBalance += _amount;
        emit DepositToContract(msg.sender, _amount, contractBalance);
    }

    modifier betConditions(uint256 _betAmount) {
        require(_betAmount <= getContractBalance() / 2, "Can't bet more than half the contract's balance!");
        require(!playersByAddress[msg.sender].betOngoing, "Bet already ongoing with this address");
        _;
    }

    function bet(uint256 _betChoice, uint256 _betAmount) public betConditions(_betAmount) {
        require(_betChoice == 0 || _betChoice == 1, "Must be either 0 or 1");
        uint256 userWalletBalance = tmng.balanceOf(msg.sender);
        require(userWalletBalance > _betAmount, "User doesn't have enough balance.");
        tmng.transferFrom(msg.sender, address(this), _betAmount);
    ...
    function random() private view returns (bool) {
        uint256 _randomResult = uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp))) % 100;
        if (_randomResult < 40) return true;
        else return false;
    }
    ...
    function withdrawPlayerBalance() external {
        ...
        tmng.transfer(msg.sender, amount);
    ...
    function _payout(address to) private returns (uint256) {
        require(contractBalance != 0, "No funds to withdraw");

        contractBalance = 0;
        tmng.transfer(to, contractBalance);
        return contractBalance;
    }
}
